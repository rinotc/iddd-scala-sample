...
class CalendarEntryInviteesSpec extends FixtureAnyFlatSpec with Matchers with AutoRollback {
  ...
  it should "find by where clauses" in { implicit session =>
    val maybeFound = CalendarEntryInvitees.findBy(sqls.eq(cei.calendarEntryInviteesId, 123))
    maybeFound.isDefined should be(true)
  }
  ...
  it should "find all by where clauses" in { implicit session =>
    val results = CalendarEntryInvitees.findAllBy(sqls.eq(cei.calendarEntryInviteesId, 123))
    results.size should be >(0)
  }
  it should "count by where clauses" in { implicit session =>
    val count = CalendarEntryInvitees.countBy(sqls.eq(cei.calendarEntryInviteesId, 123))
    count should be >(0L)
  }
